    ...
    if (clientsListEl) {
        clientsListEl.addEventListener('click', (event) => {
            ...
            if (item && clientsListEl.contains(item)) {
                ...
                try {
                    console.log('[ADMIN] typeof showClientDetails', typeof showClientDetails, typeof window.showClientDetails);
                    (window.showClientDetails || showClientDetails)(clientId);
                } catch (e) {
                    console.error('[ADMIN] showClientDetails call failed', e);
                }
            }
        });
    }
...
async function showClientDetails(clientId) {
    console.log('[ADMIN] showClientDetails start', clientId);
    let client = clients.find(c => c.id === clientId);
    if (!client) {
        client = await window.firebaseService.getUserById(clientId);
        console.log('[ADMIN] fetched client from DB', client);
    }
    if (!client) {
        console.warn('[ADMIN] client not found', clientId);
        return;
    }
    
    try {
        // Показать модалку сразу с лоадером
