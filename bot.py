...
if __name__ == "__main__":
    ...
    try:
        # 1) Сбросить вебхук и удалить висящие апдейты
        bot.delete_webhook(drop_pending_updates=True)
        print("✅ Webhook сброшен (drop_pending_updates=True)")

        # 2) Стартовать один-единственный лонг-поллинг
        print("🔄 Запуск polling...")
        bot.infinity_polling(timeout=60, long_polling_timeout=60, skip_pending=True)
    except Exception as e:
        print(f"❌ Ошибка: {e}")
        print("💡 Попробуйте остановить все запущенные экземпляры бота и запустить снова.")
